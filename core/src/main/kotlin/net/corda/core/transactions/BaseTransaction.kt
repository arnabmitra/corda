...
abstract class BaseTransaction(
    ...
    fun <T : ContractState> outputsOfType(clazz: Class<T>): List<T> {
        @Suppress("UNCHECKED_CAST")
        return outputs.filter { clazz.isAssignableFrom(it.data.javaClass) }.map { it.data as T }
    }
    ...
    fun <T : ContractState> outRefsOfType(clazz: Class<T>): List<StateAndRef<T>> {
        ...
        return outputs.mapIndexed { index, state -> StateAndRef(state, StateRef(id, index)) }
                .filter { clazz.isAssignableFrom(it.state.data.javaClass) }
 ...
 */
inline fun <reified T : ContractState> BaseTransaction.outputsOfType(): List<T> = this.outputsOfType(T::class.java)
...
inline fun <reified T : ContractState> BaseTransaction.outRefsOfType(): List<StateAndRef<T>> = this.outRefsOfType(T::class.java)
