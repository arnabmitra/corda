...
class LedgerTransaction(
     ...
     * @param clazz The class type used for filtering via an isAssignable check.
    ...
    fun <T : ContractState> inputsOfType(clazz: Class<T>): List<T> {
        @Suppress("UNCHECKED_CAST")
        return inputs.map { it.state.data }.filter { clazz.isAssignableFrom(it.javaClass) }.map { it as T }
    }
    ...
    fun <T : ContractState> inRefsOfType(clazz: Class<T>): List<StateAndRef<T>> {
        @Suppress("UNCHECKED_CAST")
        return inputs.filter { clazz.isAssignableFrom(it.state.data.javaClass) }.map { it as StateAndRef<T> }
    }
    ...
    fun <T : CommandData> commandsOfType(clazz: Class<T>): List<Command> {
        return commands.filter { clazz.isAssignableFrom(it.value.javaClass) }.map { Command(it.value, it.signers) }
    }
 ...
 */
inline fun <reified T : ContractState> LedgerTransaction.inputsOfType(): List<T> = this.inputsOfType(T::class.java)

inline fun <reified T : ContractState> LedgerTransaction.inRefsOfType(): List<StateAndRef<T>> = this.inRefsOfType(T::class.java)
...
inline fun <reified T : CommandData> LedgerTransaction.commandsOfType(): List<Command> = this.commandsOfType(T::class.java)

inline fun <reified T : CommandData> LedgerTransaction.filterCommands(crossinline predicate: (T) -> Boolean): List<Command> {
    return filterCommands(Predicate { predicate(it) }, T::class.java)
}

inline fun <reified T : CommandData> LedgerTransaction.findCommand(crossinline predicate: (T) -> Boolean): Command {
